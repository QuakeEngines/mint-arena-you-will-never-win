...
#define ART_MAP_COMPLETE6			"menu/art/level_complete6"
...
static void SetSkillColor( int skill, vec4_t color ) {
	...
	case 5:
		...
		break;
	case 6:
		skillMenuInfo.item_neverwin.color = color;
		break;
	default:
		break;
	}
...
static void UI_SPSkillMenu_SkillEvent( void *ptr, int notification ) {
	...
	else if( id == ID_NEVERWIN ) {
		trap_S_StartLocalSound( skillMenuInfo.neverwinSound, CHAN_ANNOUNCER );
	}
	else {
...
void UI_SPSkillMenu_Cache( void ) {
	...
	skillMenuInfo.skillpics[5] = trap_R_RegisterShaderNoMip( ART_MAP_COMPLETE6 );
	skillMenuInfo.neverwinSound = trap_S_RegisterSound( "sound/misc/neverwin.wav", qfalse );
...
static void UI_SPSkillMenu_Init( void ) {
	...
	skillMenuInfo.item_neverwin.generic.type		= MTYPE_PTEXT;
	skillMenuInfo.item_neverwin.generic.flags		= QMF_CENTER_JUSTIFY|QMF_PULSEIFFOCUS;
	skillMenuInfo.item_neverwin.generic.x			= 320;
	skillMenuInfo.item_neverwin.generic.y			= 311;
	skillMenuInfo.item_neverwin.generic.callback	= UI_SPSkillMenu_SkillEvent;
	skillMenuInfo.item_neverwin.generic.id			= ID_NEVERWIN;
	skillMenuInfo.item_neverwin.string				= "YOU WILL NEVER WIN!";
	skillMenuInfo.item_neverwin.color				= text_big_color;
	skillMenuInfo.item_neverwin.style				= UI_CENTER;
	...
	Menu_AddItem( &skillMenuInfo.menu, ( void * )&skillMenuInfo.item_neverwin );
